...
def test_analysis(n_rows=5):
    ...
    results = analyze_data(max_rows=n_rows, data_file='test_data.csv')
    
    if results is not None:
        print("\nQualified entries found:")
        for i, row in results.iterrows():
            print(f"\nEntry {i+1}:")
            print(f"Context: {row['decision_context']}")
            print(f"Activity Type: {row['activity_type']}")
            print(f"Final Choice: {row['final_choice']}")
            print(f"Decision Factors: {row['decision_factors']}")
            if 'reasoning' in row:
